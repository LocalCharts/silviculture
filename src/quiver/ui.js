...
export class UI {
    constructor(element) {
        ...
        this.rawElement = element.element;
        ...
        // The size of the view (i.e. the this.rawElement dimensions).
        this.dimensions = new Dimensions(this.rawElement.offsetWidth, this.rawElement.offsetHeight);
        ...
        this.shortcuts.add([
            ...
            if (this.in_mode(UIMode.Default)) {
                ...
                const view = new Dimensions(
                    this.rawElement.offsetWidth / 2 ** this.scale,
                    this.rawElement.offsetHeight / 2 ** this.scale,
    ...
    offset_from_event(event) {
        ...
        const rect = this.rawElement.getBoundingClientRect()
        return new Offset(event.pageX - rect.left, event.pageY - rect.top)
            .sub(new Offset(this.rawElement.offsetWidth / 2, this.rawElement.offsetHeight / 2))
            .div(scale)
            .add(this.view);
    }
    ...
    scale_to_fit() {
        ...
        const window_width = Math.max(0, this.rawElement.clientWidth - 2 * CONSTANTS.EMBED_PADDING);
        const window_height = Math.max(0, this.rawElement.clientHeight - 2 * CONSTANTS.EMBED_PADDING);
    ...
    static display_error(message, type = null) {
        const body = new DOM.Element(this.rawElement);
    ...
    static dismiss_error(type = null) {
        const error = new DOM.Element(this.rawElement).query_selector(`.error-banner${
    ...
    initialise_grid(element) {
        const [width, height] = [this.rawElement.offsetWidth, this.rawElement.offsetHeight];
    ...
    update_grid() {
        ...
        const rect = this.rawElement.getBoundingClientRect()
        const offset = this.view.add(new Offset(rect.left, -rect.top));
...
class Panel {
    ...
    initialise(ui) {
        ...
        this.rawElement = this.element.element
    ...
    update_position() {
        ...
        const document_height = this.rawElement.offsetHeight;
...
// We want until the (minimal) DOM content has loaded, so we have access to `this.rawElement`.
